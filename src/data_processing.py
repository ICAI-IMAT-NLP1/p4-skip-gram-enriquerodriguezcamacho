...
def subsample_words(words: List[str], vocab_to_int: Dict[str, int], threshold: float = 1e-5) -> Tuple[List[int], Dict[str, float]]:
    ...
    for word in words:
        freq = freqs[word]
        p_keep = ((freq / threshold) ** 0.5 + 1) * (threshold / freq)
        if p_keep > 1:
            p_keep = 1
...
def get_target(words: List[str], idx: int, window_size: int = 5) -> List[str]:
    ...
    target_words = [words[i] for i in range(start, end) if i != idx]
    return target_words
...
def get_batches(words: List[int], batch_size: int, window_size: int = 5) -> Generator[Tuple[List[int], List[int]], None, None]:
    ...
    for i in range(0, len(words) - batch_size + 1, batch_size):
        ...
        for j in range(i, i + batch_size):
            context = get_target(words, j, window_size)
            for tgt in context:
                inputs.append(words[j])
                targets.append(tgt)
...
def cosine_similarity(embedding: torch.nn.Embedding, valid_size: int = 16, valid_window: int = 100, device: str = 'cpu'):
    ...
    # TODO
    valid_examples = torch.randint(low=0, high=valid_window, size=(valid_size,))
    embed_weights = embedding.weight.to(device)
    norm = embed_weights.norm(dim=1, keepdim=True)
    normalized_embeddings = embed_weights / norm
    valid_embeddings = normalized_embeddings[valid_examples]
    similarities = torch.mm(valid_embeddings, normalized_embeddings.t())
